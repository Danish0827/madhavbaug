  ...
  return (
    <div className="relative">
      ...
      <Swiper
        ...
        spaceBetween={24}
        slidesPerView={1.1}
        breakpoints={{ 640: { slidesPerView: 2 }, 1024: { slidesPerView: 3 }, 1280: { slidesPerView: 3.4 } }}
      >
        {items.map((a, i) => (
          <SwiperSlide key={i} className="h-auto pb-2">
            <article className="flex h-full flex-col overflow-hidden rounded-[28px] shadow-sm ring-1 ring-black/5">
              <div className="bg-brand-gradient flex h-44 items-center justify-center">
                <Trophy className="h-14 w-14 text-white/90" />
              </div>
              <div className="flex flex-1 flex-col bg-gradient-to-br from-[#006589]/[0.05] to-[#3d4281]/[0.05] p-6">
                <div className="flex items-start justify-between gap-3">
                  <h3 className="font-display text-lg leading-snug text-teal-deep">{a.title}</h3>
                  <span className="shrink-0 rounded-full bg-gradient-to-br from-[#006589] to-[#3d4281] px-3 py-1 text-xs font-semibold text-white">
                    {a.year}
                  </span>
                </div>
